...
import {
  ...
  CharacterStore,
  LeagueStore
} from "../shared/interfaces/settings-store.interface";
...
export class LoginComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private deprecationDialog: MatDialog
  ) {
    if (this.electronService.isElectron()) {
      this.lineReader = window.require("read-last-lines");
    }

    this.leaguesSub = this.externalService.leagues.subscribe(
      (res: League[]) => {
        this.leagues = res;
      }
    );
    ...
    this.leagueFormGroup = fb.group({
      leagueName: [
        this.leagueName !== undefined ? this.leagueName : "",
        Validators.required
      ],
      tradeLeagueName: [
        this.tradeLeagueName !== undefined ? this.tradeLeagueName : "",
        Validators.required
      ]
    });
    ...
    this.charFormGroup = fb.group({
      characterName: [
        this.characterName !== undefined ? this.characterName : "",
        Validators.required
      ]
    });
    this.pathFormGroup = fb.group({
      filePath: [
        this.filePath !== undefined ? this.filePath : "",
        Validators.required
      ]
    });

    this.logMonitorService.trackMapsOnly =
      this.logMonitorService.trackMapsOnly !== undefined
        ? this.logMonitorService.trackMapsOnly
        : true;
  }

  openDeprecationDialog(): void {
    if (this.electronService.isElectron()) {
      const dialogRef = this.deprecationDialog.open(DeprecationDialogComponent, {
        width: "650px",
        data: {
          icon: "warning",
    ...
    if (this.electronService.isElectron()) {
      this.pathValid =
        this.electronService.fs.existsSync(
          this.pathFormGroup.controls.filePath.value
        ) &&
        this.pathFormGroup.controls.filePath.value
          .toLowerCase()
          .endsWith("client.txt");
  ...
  getRequiredState() {
    setTimeout(() => {
      return (
        this.privateProfileError ||
        (!this.sessionIdValid &&
          !this.needsValidation &&
          this.sessFormGroup.controls.sessionId.value !== "")
      );
    });
  }
  ...
  checkLeagues() {
    ...
    if (accName !== undefined && accName !== "") {
      const request = forkJoin(
        this.externalService.getCharacterList(
          accName,
      ...
      request.subscribe(res => {
        ...
        if (
          distinctLeagues.find(l => l.id === this.leagueName) === undefined ||
  ...
  getLeagues(accountName?: string, skipStep?: boolean, shouldValidate = false) {
    ...
    if (accName !== undefined && accName !== "") {
      ...
      request.subscribe(res => {
        ...
        if (
          res[0].errorType !== undefined &&
          res[0].errorType === ErrorType.Unauthorized
        ) {
          // profile is private
          ...
          res[0].errorType !== undefined &&
          res[0].errorType === ErrorType.Unreachable
        ) {
    ...
    this.externalService
      ...
      .subscribe(
        res => {
          ...
          if (
            res.errorType !== undefined &&
            res.errorType === ErrorType.Unauthorized
          ) {
            this.openErrorMsgDialog({
              title: "Could not fetch characters",
              // tslint:disable-next-line:max-line-length
              body:
                "Character-list could not be fetched. This is most likely caused from your character-list being hidden.<br/><br/>" +
          ...
          if (
            res.errorType !== undefined &&
            res.errorType === ErrorType.Unreachable
          ) {
            this.siteUnreachable = true;
          } else {
            const charactersByLeague = res.filter(
              x => x.league === this.leagueFormGroup.controls.leagueName.value
            );
            this.accountService.characterList.next(charactersByLeague);

            if (
              this.characterList.find(x => x.name === this.characterName) ===
              undefined
            ) {
  ...
  likelyhoodCheck(filePath) {
    if (this.lineReader !== undefined) {
      this.lineReader.read(filePath, 2).then(lines => {
        ...
        if (twoDaysAgo > lastTimestamp) {
          const errorMsg = {
            ...
            body:
              "We detected that your Client.txt contains older data.<br/><br/>" +
  ...
  login() {
    ...
    const request = forkJoin(
      this.externalService.getCharacterInventory(
  ...
  sendValidateRequests(requests: any) {
    forkJoin(requests).subscribe(results => {
      ...
      this.sessionIdValid =
        successfulRequest !== false && successfulRequest !== undefined;
    });
  }
  ...
  completeLogin() {
    ...
    this.externalService
      ...
      .subscribe(res => {
        ...
        let characters: CharacterStore[] = this.settingsService.get(
          "characters"
        );
        const newCharacter = {
          name: this.form.characterName,
          areas: this.areaHistory,
          networth: this.netWorthHistory
        } as CharacterStore;
        if (characters === undefined) {
          characters = [newCharacter];
        }
        if (
          characters.find(l => l.name === this.form.characterName) === undefined
        ) {
        ...
        if (
          this.tradeLeagueName !==
          this.leagueFormGroup.controls.tradeLeagueName.value
        ) {
        ...
        this.player.pastAreas = HistoryHelper.filterAreas(
          this.areaHistory,
          oneDayAgo
        );
        ...
        this.settingsService.set(
          "lastLeague",
          this.leagueFormGroup.controls.leagueName.value
        );
